                ...
                templateFn = input => {
                    ...
                    Alpine.dontAutoEvaluateFunctions(() => {
                        evaluator(value => {
                            ...
                            '$decimal': formatDecimal.bind({ el }),
                            '$integer': formatInteger.bind({ el })
                        }})
                    })
...
export function stripDown(template, input) {
    ...
    for (let i = 0; i < template.length; i++) {
        if (WILDCARDS.includes(template[i])) {
...
export function buildUp(template, input) {
    ...
    for (let i = 0; i < template.length; i++) {
        ...
        output += transformation[template[i]](clean.shift())
    }

    return output
}

function formatMoney(input, delimiter = '.', thousands, decimals, unsigned) {
    ...
    let sign = unsigned ? '' : (input[0]==='-' || input[0]==='+' ? input[0] : '')
    let strippedInput = input.replaceAll(new RegExp(`[^0-9\\${delimiter}]`, 'g'), '')
    let template = Array.from({ length: strippedInput.split(delimiter)[0].length || 1 }).fill('9').join('')

    template = sign + addThousands(template, thousands)

    if (input.includes(delimiter)) template += delimiter + Array.from({ length: decimals || 2 }).fill('9').join('')
...
function formatDecimal(input, delimiter = '.', thousands, decimals, unsigned) {
    thousands = thousands ?? (delimiter === "," ? "." : ",")
...
function formatInteger(input, thousands = ',', unsigned = true) {
    ...
    let sign = unsigned ? '' : (input[0]==='-' || input[0]==='+' ? input[0] : '')
    let strippedInput = input.replaceAll(/[^0-9]/g, '')
    let template = Array.from({ length: strippedInput.length || 1 }).fill('9').join('')
